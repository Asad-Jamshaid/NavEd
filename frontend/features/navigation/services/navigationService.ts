...
import { Coordinate, NavigationRoute, NavigationStep, Building } from '../../../shared/types';
import { BUILDINGS, ROOMS, searchBuildings, searchRooms, getAccessibleBuildings } from '../../../shared/data/campusData';
...
function aStarPathfinding(
  ...
  // #region agent log
  console.log('[A*] Starting pathfinding:', { startId, endId });
  fetch('http://127.0.0.1:7242/ingest/9d1aafa7-8f4f-466c-980b-f7e8c76c04b5',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'navigationService.ts:176',message:'aStarPathfinding started',data:{startId,endId,graphSize:graph.size},timestamp:Date.now(),sessionId:'debug-session',runId:'post-fix',hypothesisId:'A'})}).catch(()=>{});
  // #endregion
  ...
  if (!startWp || !endWp) {
    // #region agent log
    console.error('[A*] Invalid waypoints:', { startWp: !!startWp, endWp: !!endWp });
    // #endregion
    return null;
  }
  ...
  const closedSet = new Set<string>(); // Track processed nodes to prevent revisiting
  ...
  while (openSet.size > 0) {
    ...
    if (iterations > maxIterations) {
      // #region agent log
      console.error('[A*] Max iterations reached:', { iterations, openSetSize: openSet.size, startId, endId });
      fetch('http://127.0.0.1:7242/ingest/9d1aafa7-8f4f-466c-980b-f7e8c76c04b5',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'navigationService.ts:212',message:'aStarPathfinding max iterations reached',data:{startId,endId,openSetSize:openSet.size,iterations},timestamp:Date.now(),sessionId:'debug-session',runId:'post-fix',hypothesisId:'A'})}).catch(()=>{});
      // #endregion
      break;
    }
    ...
    if (iterations % 100 === 0) {
      console.log('[A*] Iteration:', iterations, 'openSet:', openSet.size);
    }
    ...
    // Guard against infinite loop: if no valid node found, break
    if (!current || current === '' || lowestF === Infinity) {
      // #region agent log
      console.error('[A*] Invalid current node:', { current, lowestF, openSetSize: openSet.size, iterations });
      fetch('http://127.0.0.1:7242/ingest/9d1aafa7-8f4f-466c-980b-f7e8c76c04b5',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'navigationService.ts:230',message:'aStarPathfinding invalid current node',data:{current,lowestF,openSetSize:openSet.size,iterations},timestamp:Date.now(),sessionId:'debug-session',runId:'post-fix',hypothesisId:'A'})}).catch(()=>{});
      // #endregion
      break;
    }
    ...
    if (current === endId) {
      ...
      // #region agent log
      console.log('[A*] Path found:', { pathLength: path.length, iterations });
      // #endregion
      return path;
    }
...
export async function getRoute(
  ...
  // #region agent log
  console.log('[getRoute] Called with:', { from, to, accessibleOnly });
  fetch('http://127.0.0.1:7242/ingest/9d1aafa7-8f4f-466c-980b-f7e8c76c04b5',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'navigationService.ts:271',message:'getRoute called',data:{fromLat:from.latitude,fromLng:from.longitude,toLat:to.latitude,toLng:to.longitude,accessibleOnly},timestamp:Date.now(),sessionId:'debug-session',runId:'post-fix',hypothesisId:'A'})}).catch(()=>{});
  // #endregion
  try {
    // Find nearest waypoints
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/9d1aafa7-8f4f-466c-980b-f7e8c76c04b5',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'navigationService.ts:257',message:'calling findNearestWaypoint for start',data:{fromLat:from.latitude,fromLng:from.longitude},timestamp:Date.now(),sessionId:'debug-session',runId:'init',hypothesisId:'B'})}).catch(()=>{});
    // #endregion
    const startWp = findNearestWaypoint(from);
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/9d1aafa7-8f4f-466c-980b-f7e8c76c04b5',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'navigationService.ts:258',message:'findNearestWaypoint start completed',data:{startWpId:startWp?.id},timestamp:Date.now(),sessionId:'debug-session',runId:'init',hypothesisId:'B'})}).catch(()=>{});
    // #endregion
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/9d1aafa7-8f4f-466c-980b-f7e8c76c04b5',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'navigationService.ts:259',message:'calling findNearestWaypoint for end',data:{toLat:to.latitude,toLng:to.longitude},timestamp:Date.now(),sessionId:'debug-session',runId:'init',hypothesisId:'B'})}).catch(()=>{});
    // #endregion
    const endWp = findNearestWaypoint(to);
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/9d1aafa7-8f4f-466c-980b-f7e8c76c04b5',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'navigationService.ts:260',message:'findNearestWaypoint end completed',data:{endWpId:endWp?.id},timestamp:Date.now(),sessionId:'debug-session',runId:'init',hypothesisId:'B'})}).catch(()=>{});
    // #endregion

    // Build graph
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/9d1aafa7-8f4f-466c-980b-f7e8c76c04b5',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'navigationService.ts:263',message:'calling buildGraph',data:{accessibleOnly},timestamp:Date.now(),sessionId:'debug-session',runId:'init',hypothesisId:'C'})}).catch(()=>{});
    // #endregion
    const graph = buildGraph(accessibleOnly);
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/9d1aafa7-8f4f-466c-980b-f7e8c76c04b5',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'navigationService.ts:264',message:'buildGraph completed',data:{graphSize:graph?.size||0},timestamp:Date.now(),sessionId:'debug-session',runId:'init',hypothesisId:'C'})}).catch(()=>{});
    // #endregion

    // Find path using A*
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/9d1aafa7-8f4f-466c-980b-f7e8c76c04b5',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'navigationService.ts:267',message:'calling aStarPathfinding',data:{startId:startWp.id,endId:endWp.id},timestamp:Date.now(),sessionId:'debug-session',runId:'init',hypothesisId:'A'})}).catch(()=>{});
    // #endregion
    console.log('[getRoute] Calling aStarPathfinding...');
    const path = aStarPathfinding(startWp.id, endWp.id, graph);
    // #region agent log
    console.log('[getRoute] aStarPathfinding completed:', { pathLength: path?.length || 0, hasPath: !!path });
    fetch('http://127.0.0.1:7242/ingest/9d1aafa7-8f4f-466c-980b-f7e8c76c04b5',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'navigationService.ts:310',message:'aStarPathfinding completed',data:{pathLength:path?.length||0,hasPath:!!path},timestamp:Date.now(),sessionId:'debug-session',runId:'post-fix',hypothesisId:'A'})}).catch(()=>{});
    // #endregion
