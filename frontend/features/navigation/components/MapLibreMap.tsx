...
// Esri World Imagery - FREE satellite tiles (similar to Google Earth)
...
export const MapView = forwardRef<MapLibreMapRef, MapLibreMapProps>((props, ref) => {
  ...
  const [currentMapStyle, setCurrentMapStyle] = useState<string | object>(MAP_STYLES.openFreeMap);
  const [routeCoordinates, setRouteCoordinates] = useState<Coordinate[]>([]);
  const [routeColor, setRouteColor] = useState<string>('#1E3A5F');
  const [routeWeight, setRouteWeight] = useState<number>(5);
  ...
  useImperativeHandle(ref, () => ({
    ...
    changeMapStyle: (style: 'light' | 'dark' | 'satellite' | 'terrain') => {
      ...
      if (style === 'satellite') {
        setCurrentMapStyle(ESRI_SATELLITE_STYLE);
      } else {
        const styleUrls: Record<string, string> = {
          light: MAP_STYLES.openFreeMap,
          dark: 'https://tiles.openfreemap.org/styles/dark',
          terrain: MAP_STYLES.versatiles,
        };
        setCurrentMapStyle(styleUrls[style] || MAP_STYLES.openFreeMap);
      }
    },
    drawRoute: (coordinates: Coordinate[], color = '#1E3A5F', weight = 5) => {
      setRouteCoordinates(coordinates);
      setRouteColor(color);
      setRouteWeight(weight);

      // Fit map to show the entire route - reuse fitToCoordinates logic
...
export const Marker: React.FC<MarkerProps> = ({
  ...
  // Use PointAnnotation instead of MarkerView for better positioning
