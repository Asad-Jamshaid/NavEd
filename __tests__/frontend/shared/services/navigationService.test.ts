...
describe('NavigationService', () => {
  ...
  describe('formatDuration', () => {
    ...
    it('should format durations 1 hour or more', () => {
      ...
      expect(formatDuration(135 * 60)).toBe('2h 15m');
    });
    ...
    it('should handle large durations', () => {
      expect(formatDuration(300)).toBe('5 min'); // 300 seconds = 5 minutes
      expect(formatDuration(1440)).toBe('24 min'); // 1440 seconds = 24 minutes
      expect(formatDuration(3600)).toBe('1h 0m'); // 3600 seconds = 60 minutes = 1 hour
      expect(formatDuration(3660)).toBe('1h 1m'); // 3660 seconds = 61 minutes = 1 hour, 1 minute
      expect(formatDuration(18000)).toBe('5h 0m'); // 18000 seconds = 300 minutes = 5 hours
      expect(formatDuration(86400)).toBe('24h 0m'); // 86400 seconds = 1440 minutes = 24 hours
    });
  });
