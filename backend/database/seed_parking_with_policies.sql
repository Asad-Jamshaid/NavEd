-- ==========================================
-- Complete Parking Data Seed Script with RLS Policies
-- This script:
-- 1. Adds INSERT policies for parking_lots (if needed)
-- 2. Inserts Parking Lots
-- 3. Inserts Parking History
-- Run this in your Supabase SQL Editor
-- ==========================================

-- ==========================================
-- STEP 1: Add INSERT policy for parking_lots (if it doesn't exist)
-- ==========================================

-- Allow service role to insert parking lots (for seeding)
...
BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Service role can insert parking lots'
  ) THEN
    CREATE POLICY "Service role can insert parking lots" ON public.parking_lots
      FOR INSERT 
      WITH CHECK (auth.role() = 'service_role' OR auth.role() = 'authenticated');
  END IF;
END $$;

-- Also allow authenticated users to insert (for app usage)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'parking_lots' 
    AND policyname = 'Authenticated users can insert parking lots'
  ) THEN
    CREATE POLICY "Authenticated users can insert parking lots" ON public.parking_lots
      FOR INSERT 
